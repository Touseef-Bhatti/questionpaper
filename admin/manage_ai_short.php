        ...
        if ($action === 'update') {
            ...
            if ($id > 0) {
                ...
                if ($question === '' || $answer === '') {
                    ...
                    $stmt = $conn->prepare(
                        "UPDATE AIGeneratedShortQuestions 
                    ...
                    if ($stmt) {
                        ...
                        if ($stmt->execute()) {
                            ...
                            logAdminAction('update_ai_short', "ID: $id");
                        } else {
            ...
            if ($id > 0) {
                ...
                $topicName = '';
                $getTopic = $conn->prepare("SELECT t.topic_name FROM AIGeneratedShortQuestions q JOIN AIQuestionsTopic t ON q.topic_id = t.id WHERE q.id = ?");
                if ($getTopic) {
                ...
                $stmt = $conn->prepare("DELETE FROM AIGeneratedShortQuestions WHERE id = ?");
                if ($stmt) {
                    $stmt->bind_param('i', $id);
                    if ($stmt->execute()) {
                        $message = 'Short Question deleted successfully.';
                        logAdminAction('delete_ai_short', "ID: $id");
                        
                        // Update Count
                        if ($topicName) {
                            $upd = $conn->prepare("UPDATE TopicShortQuestionCounts SET question_count = GREATEST(0, question_count - 1) WHERE topic_name = ?");
                            if ($upd) {
...
$overallTotal = 0;
$totalRes = $conn->query("SELECT COUNT(*) AS cnt FROM AIGeneratedShortQuestions");
...
<head>
    ...
    <title>Manage AI Short Questions</title>
...
<body>
    ...
    <div class="ai-container">
        ...
        <div class="ai-header">
            <h1>Manage AI Short Questions</h1>
        </div>
        ...
        <div class="ai-summary">
            ...
            <div class="ai-card">
                ...
                <a href="manage_ai_short.php" style="color:#007bff;">Clear filter</a>
            </div>
            <?php endif; ?>
            <div class="ai-card">
                <h3>Rename Topic</h3>
