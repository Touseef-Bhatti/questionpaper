...
// Pre-process chapter IDs and question IDs for JS
$selectedChapterIds = explode(',', $exam['chapter_ids']);
$selectedQuestionIds = json_decode($exam['question_ids'], true) ?: [];
?>
                ...
                <div class="card shadow mb-4">
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="title" class="form-control" value="<?= htmlspecialchars($exam['title']) ?>" placeholder="e.g. Midterm Test - Chapter 1 & 2" required>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="class_id" id="class_id" class="form-select" required>
                                    ...
                                    <?php while ($c = $classes->fetch_assoc()): ?>
                                        <option value="<?= $c['class_id'] ?>" <?= $exam['class_id'] == $c['class_id'] ? 'selected' : '' ?>><?= htmlspecialchars($c['class_name']) ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="book_id" id="book_id" class="form-select" required>
                                    ...
                                    <?php foreach ($bookOptions as $bk): ?>
                                        <option value="<?= $bk['book_id'] ?>" data-class="<?= $bk['class_id'] ?>" <?= $exam['book_id'] == $bk['book_id'] ? 'selected' : '' ?>><?= htmlspecialchars($bk['book_name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-5">
                                ...
                                <div id="chapter-list" class="border rounded" style="max-height: 500px; overflow-y: auto; background: #f8f9fa;">
                                    <p class="text-muted p-3">Loading chapters...</p>
                                </div>
                            </div>
                            <div class="col-md-7">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <div class="btn-group w-100" role="group">
                                    <input type="radio" class="btn-check" name="selection_type" id="type_random" value="random" <?= $exam['selection_type'] === 'random' ? 'checked' : '' ?>>
                                    <label class="btn btn-outline-primary" for="type_random">System Random</label>
                                    
                                    <input type="radio" class="btn-check" name="selection_type" id="type_manual" value="manual" <?= $exam['selection_type'] === 'manual' ? 'checked' : '' ?>>
                        ...
                        <div id="random-settings">
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="number" name="mcq_count" class="form-control" value="<?= $exam['mcq_count'] ?>" min="0">
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label">No. of Short Qs</label>
                                    <input type="number" name="short_count" class="form-control" value="<?= $exam['short_count'] ?>" min="0">
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label class="form-label">No. of Long Qs</label>
                                    <input type="number" name="long_count" class="form-control" value="<?= $exam['long_count'] ?>" min="0">
                                </div>
                            </div>
                            <div class="mt-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let selectedQuestionIds = new Set(<?= json_encode($selectedQuestionIds) ?>);
    let initialChapterIds = <?= json_encode($selectedChapterIds) ?>;
    const chapterQuestionsCache = {};
    const fetchingChapters = new Set();

    // Initial load
    if (classSel.value) {
        sumClass.textContent = classSel.options[classSel.selectedIndex].text;
        filterBooks(classSel.value);
    }
    if (bookSel.value) {
        sumBook.textContent = bookSel.options[bookSel.selectedIndex].text;
        loadChapters(bookSel.value, classSel.value);
    }

    function filterBooks(clsId) {
    ...
    async function loadChapters(bookId, clsId) {
        if (!bookId || !clsId) return;
        ...
        try {
            ...
            if (bookChapters.length === 0) {
                ...
                bookChapters.forEach(ch => {
                    ...
                    const isChecked = initialChapterIds.includes(ch.chapter_id.toString());
                    item.innerHTML = `
                        <div class="d-flex align-items-center">
                            <input class="form-check-input chapter-checkbox me-2" type="checkbox" name="chapter_ids[]" value="${ch.chapter_id}" id="ch_${ch.chapter_id}" ${isChecked ? 'checked' : ''}>
    ...
    bookSel.addEventListener('change', function() {
        ...
        sumBook.textContent = this.options[this.selectedIndex].text;
